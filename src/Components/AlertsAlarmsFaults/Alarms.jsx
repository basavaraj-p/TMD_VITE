...
const AllAlarams = `subscription($siloId:Int!,$date:Date!)  {
  allAlerts(condition: {siloId: $siloId}, filter: {alertId: {lessThanOrEqualTo: 3},date: {equalTo:$date}},orderBy: START_DESC) {
    nodes {
      title
      start
      color
                  ...
                  alarams
                    ...
                    .map((alarm) => (
                      <TableRow key={alarm.siloId}>
                        ...
                        <TableCell align="center">{parseFloat(alarm.value).toFixed(2)}</TableCell>
                      </TableRow>
                    ))}
              </TableBody>
            </Table>
